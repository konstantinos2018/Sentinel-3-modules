...
# filtering windows using the manual approach for the SRAL.
# =============================================================================
...
#    # Compute percentiles
    ...
    dst = S3postproc.sral_dist(xsr[idx], ysr[idx])
    ...
    for wind, lab, prop in zip(window_size, labels, line_props):
        ...
        if lab == 'Original':
            ...
            var[lab] = [np.convolve(ssha_m, np.ones((wind))/float(wind), mode='same'),
               prop]
            legend_labels.append(lab)
...
#    dst_sr_nan, ssha_m_nan, idx_nan = S3postproc.sral_dist_nans(dst_sr, ssha_m, threshold=3*340)
    ...
    # Plot SRAL
    S3plots.multiple_cross_sections(var, dst, legend_labels, fdate)
